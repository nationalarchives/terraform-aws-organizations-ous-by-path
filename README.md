...
| <a name="input_organization_id"></a> [organization\_id](#input\_organization\_id) | The ID of the AWS Organization. If not provided, the module will attempt to discover it using a data source which could prevent Terraform from planning if the Organization is to be created as part of the same deployment. Has no effect when the module is used as a data source. | `string` | `null` | no |
| <a name="input_organization_root_id"></a> [organization\_root\_id](#input\_organization\_root\_id) | The Root ID of the AWS Organization. If not provided, the module will attempt to discover it using a data source which could prevent Terraform from planning if the Organization is to be created as part of the same deployment. Has no effect when the module is used as a data source. | `string` | `null` | no |
