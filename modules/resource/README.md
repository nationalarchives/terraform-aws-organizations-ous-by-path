...
### Renaming and moving OUs

Each OU is tracked by its full `name_path` (for example `Workloads/Application/Production`). This gives stable, readable addressing, but it has an important consequence:
...
```hcl
moved {
  from = module.ous.module.l2.aws_organizations_organizational_unit.ous["Workloads:::Application"]
  to   = module.ous.module.l2.aws_organizations_organizational_unit.ous["Workloads:::Apps"]
}
```
...
| <a name="input_organization_id"></a> [organization\_id](#input\_organization\_id) | The ID of the AWS Organization. If not provided, the module will attempt to discover it using a data source. | `string` | `null` | no |
| <a name="input_organization_root_id"></a> [organization\_root\_id](#input\_organization\_root\_id) | The Root ID of the AWS Organization. If not provided, the module will attempt to discover it using a data source. | `string` | `null` | no |
